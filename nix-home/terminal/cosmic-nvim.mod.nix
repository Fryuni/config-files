...
with lib; let
  ...
  cfg =
    trivial.throwIf
    (config.programs.cosmic-nvim.enable && config.programs.neovim.enable)
...
in {
  options = {
    programs.cosmic-nvim = {
      enable = mkEnableOption "Cosmic Neovim";

      features = mkOption {
        type = types.listOf types.str;
        description = ''
          Features to be enabled on Cosmic Neovim.
          The list of features is documented here:
          https://github.com/NTBBloodbath/cosmic-nvim/blob/main/docs/modules.md#features-modules
        '';
        default = [];
      };
  ...
  config = let
    ...
    #   pname = "cosmic-nvim";
    #   version = cfg.cosmic-nvim-src.rev;
    #
    #   src = pkgs.fetchFromGitHub cfg.cosmic-nvim-src;
    #
    #   strictDeps = true;
  ...
  in
    mkIf cfg.enable {
      ...
      # home.activation = {
      #   "mutable cosmic-nvim" = mkIf cfg.mutableConfig (
      ...
      # programs.cosmic-nvim.finalNvimPackage = pkgs.wrapNeovimUnstable cfg.nvimPackage
      #   (neovimConfig // {
      #     wrapRc = false;
      #   });
      programs.cosmic-nvim.finalNvimPackage = cfg.nvimPackage;
