...
const envExtract = new RegExp(['\\$(?:(', Object.keys(process.env).join('|'), ')|\\{(\\w+)\\})'].join(''), 'g');

function expandEnvVar(txt) {
    return txt.replace(envExtract, (_, nameUnsafe, nameSafe) => expandEnvVar(process.env[nameSafe || nameUnsafe]));
}
...
module.exports = {
    ...
    */
     ...
     */
    fileWorkingDirectory: process.env.HOME,
