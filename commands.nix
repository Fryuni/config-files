{
  ...
  in [
    ...
    }
    {
      ...
      command = ''
        ${bins.nvd} diff \
          ...
          ${self.outputs.homeConfigurations."lotus@note".config.home.path}
      '';
    }
    {
      name = "build";
      ...
      command = ''
        ${bins.nix} build --no-link --print-out-paths .#homeConfigurations."lotus@note".activationPackage $@
      '';
    }
    {
      name = "switch";
      ...
      command = ''
        ${homeManager} switch --flake '.#note' -b bck $@
      '';
    }
    {
      name = "update";
    ...
    # NixOS
    {
      ...
      command = ''
        sudo nixos-rebuild switch --flake '.#note' $@
      '';
    }
    {
      name = "os-test";
      ...
      command = ''
        sudo nixos-rebuild test --flake '.#note' $@
      '';
    }
    {
      name = "os-diff";
      ...
      command = ''
        ${bins.nvd} diff \
          /run/current-system \
          ${self.outputs.nixosConfigurations.note.config.system.build.toplevel}
      '';
    }
    {
      name = "os-build";
      ...
      command = ''
        echo "${self.outputs.nixosConfigurations.note.config.system.build.toplevel}"
      '';
    }
    {
      name = "os-boot";
      ...
      command = ''
        sudo nixos-rebuild boot --flake '.#note' $@
      '';
    }
    {
      ...
      help = "Apply NixOS configuration as the default boot profile, but don't load it immediately";
      command = ''
        sudo nixos-rebuild boot --flake '.#note' --option substitute false $@
      '';
    }
    {
      name = "os-purge";
